...
class MinecraftJsonApi (object):
	...
	def __createsocket(self):
		...
		try:
			self.log.debug('creating raw socket')
			sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			port = self.port +1
			self.log.debug('raw socket connecting')
	...
	def __init__(self, host='localhost', port=20059, username='admin', 
		...
		self.salt = salt
		self.__methods = []
		self.log = logging.getLogger('MINECRAFT API')
		self.log.debug('API CONNECTION CREATED')
	...
	def subscribe (self, feed):
		...
		self.log.debug('Creating stream connections for %s' %(feed))
		...
		self.log.debug('socket created, attempting connection.....')
		stream.write(url)
		stream.write('\n')
		stream.flush()
...
if __name__ == '__main__':
	...
	# Read params
 	paramDefaults = {'host': 'localhost', 'port':20059, 'username':'admin', 'password':'test', 'salt':''}
